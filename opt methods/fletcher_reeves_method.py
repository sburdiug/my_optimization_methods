...
def fletcher_reeves_method(
    ...
    Метод Флетчера-Рівса:
      g_k = grad f(x_k)
      s_0 = -g_0
      lambda_k = argmin f(x_k + lambda * s_k)
    ...
    for k in range(max_iter):
        ...
        denominator = float(np.dot(grad_k, grad_k))
        if denominator <= 1e-14:
            beta_next = 0.0
        else:
            beta_next = float(np.dot(grad_next, grad_next) / denominator)
        s_next = -grad_next + beta_next * s_k
...
def plot_fletcher_reeves_method(result: dict):
    ...
    ax.contour(X, Y, Z, levels=20, cmap="cividis", alpha=0.5)
    ax.plot(points[:, 0], points[:, 1], "o-", color="tab:orange", linewidth=2.0, markersize=5)
    ...
    ax.set_xlabel("x1")
    ax.set_ylabel("x2")
    ax.set_title("Метод Флетчера-Рівса: траєкторія")
...
def run_fletcher_reeves_method(
    fx: sp.Expr,
    ...
    for item in result["history"]:
        ...
        print(f"beta_{k + 1} = {item['beta_next']:.6f}")
        print(f"s^({k + 1}) = ({item['s_next'][0]:.3f}, {item['s_next'][1]:.3f})")
    ...
    if show_plot:
        plot_fletcher_reeves_method(result)
